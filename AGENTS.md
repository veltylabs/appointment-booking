...
- **`internal/tinytime` shim — NOT yet fixed.** As of this doc-only pass, `internal/tinytime/` still
  exists in this repo and `go.mod` still has `replace github.com/tinywasm/time => ./internal/tinytime`
  pinned to `tinywasm/time v0.4.0`. The prerequisite (`Weekday`/`MidnightUTC`/`LocalMinutesToUnixUTC`
  in the real `github.com/tinywasm/time`) has been published at `v0.5.0` (confirmed: these three
  functions exist in the upstream package today) — removing the shim is unblocked. See `docs/PLAN.md`
  Stage 0.
- **`tinywasm/context` (`tinyctx`) is imported today by `service.go` and every test file** — it is
  **not** on the whitelist above (the Five Contracts list is exhaustive: "model + router + view +
  events + orm + ddl — and nothing else from tinywasm/\* in non-test code"). Its only use in this
